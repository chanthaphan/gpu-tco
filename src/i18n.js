...
export const T = {
  en: {
    ...
    chGpu: 'GPU Technical Specifications & Node Throughput',
    gsHbm: 'HBM', gsBw: 'Bandwidth', gsFp8: 'FP8 (dense)', gsTdp: 'TDP/GPU', gsLink: 'Interconnect', gsSrc: 'Source',
    gsHow: 'How node throughput is calculated',
    gsStep1: (cal, plat) => `Calibrated benchmark: ${cal} tok/s/GPU on ${plat}, serving the GLM-4.5 anchor (32 GB active weights at FP8) — sources in the table above and CLAUDE.md.`,
    gsStep2: (active, model, f) => `Model scaling: ${model} streams ${active} GB of active weights per decode step, so the rate scales by 32 ÷ ${active} = ×${f} (clamped ×0.25–×4).`,
    gsStep2Custom: (tok) => `Custom model: factor = ${tok} ÷ 2,200 (your tok/s/GPU relative to the H200 reference).`,
    gsStep3: (cal, f, eff) => `Effective per GPU: ${cal} × ${f} = ${eff} tok/s.`,
    gsStep4: (eff, n, unit, node) => `Per ${unit}: ${eff} tok/s × ${n} GPUs = ${node} tok/s.`,
    gsStep5: (raw, node, exact, units, unit) => `Fleet: provision ${raw} tok/s (demand ÷ utilization headroom) ÷ ${node} = ${exact} → rounds up to ${units} ${unit}.`,
    gsPhysics: (bw, active, steps, batch, pf) => `Why bandwidth-bound: every decode step must stream the model's active weights from HBM once, so forward passes are capped at ${bw} TB/s ÷ ${active} GB ≈ ${steps}/sec per GPU. The calibrated rate implies ~${batch} tokens generated per pass (effective concurrent batch). FP8 compute (${pf} PFLOPS) is rarely the decode bottleneck — prefill and long contexts are where compute bites.`,
  ...
  th: {
    ...
    chGpu: 'สเปกทางเทคนิคของ GPU และการคำนวณ throughput ต่อโหนด',
    gsHbm: 'HBM', gsBw: 'แบนด์วิดท์', gsFp8: 'FP8 (dense)', gsTdp: 'TDP/GPU', gsLink: 'อินเตอร์คอนเนกต์', gsSrc: 'แหล่งอ้างอิง',
    gsHow: 'วิธีคำนวณ throughput ต่อโหนด',
    gsStep1: (cal, plat) => `ค่า benchmark ที่คาลิเบรต: ${cal} tok/s/GPU บน ${plat} เมื่อให้บริการโมเดลอ้างอิง GLM-4.5 (น้ำหนักแอคทีฟ 32 GB ที่ FP8) — แหล่งอ้างอิงในตารางด้านบน`,
    gsStep2: (active, model, f) => `ปรับตามโมเดล: ${model} ต้องอ่านน้ำหนักแอคทีฟ ${active} GB ต่อหนึ่งขั้น decode อัตราจึงคูณด้วย 32 ÷ ${active} = ×${f} (จำกัด ×0.25–×4)`,
    gsStep2Custom: (tok) => `โมเดลกำหนดเอง: ตัวคูณ = ${tok} ÷ 2,200 (เทียบกับ H200 อ้างอิง)`,
    gsStep3: (cal, f, eff) => `ความเร็วสุทธิต่อ GPU: ${cal} × ${f} = ${eff} tok/s`,
    gsStep4: (eff, n, unit, node) => `ต่อ${unit}: ${eff} tok/s × ${n} GPU = ${node} tok/s`,
    gsStep5: (raw, node, exact, units, unit) => `ทั้งฟลีต: ต้องจัดเตรียม ${raw} tok/s (ความต้องการ ÷ เป้าหมายการใช้งาน) ÷ ${node} = ${exact} → ปัดขึ้นเป็น ${units} ${unit}`,
    gsPhysics: (bw, active, steps, batch, pf) => `ทำไมจึงถูกจำกัดด้วยแบนด์วิดท์: ทุกขั้น decode ต้องอ่านน้ำหนักแอคทีฟจาก HBM หนึ่งรอบ จำนวนรอบจึงไม่เกิน ${bw} TB/s ÷ ${active} GB ≈ ${steps} รอบ/วินาที ต่อ GPU อัตราที่คาลิเบรตหมายถึง ~${batch} โทเคนต่อรอบ (จำนวนงานพร้อมกันสุทธิ) กำลังประมวลผล FP8 (${pf} PFLOPS) แทบไม่ใช่คอขวดของ decode — คอขวดด้านคำนวณอยู่ที่ prefill และคอนเท็กซ์ยาว`,
